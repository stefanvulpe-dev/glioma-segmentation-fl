...
class FedNovaWithLogging(fl.server.strategy.FedAvg):
    ...
    def aggregate_fit(self, rnd, results, failures):
        ...
        for client_proxy, fit_res in results:
            ...
            total_n += n_i
            tau_i = float(m.get("tau", 1.0))
            w_i = _params_to_ndarrays(fit_res.parameters)
            client_items.append((client_proxy, fit_res, n_i, tau_i, w_i))

        if total_n <= 0:
            return super().aggregate_fit(rnd, results, failures)

        eta = 0.0
        for _, _, n_i, tau_i, _ in client_items:
            w = n_i / total_n
            eta += w * max(tau_i, 1e-12)

        agg_u = [0.0 * w for w in self._current_weights]
        for _, _, n_i, tau_i, w_i in client_items:
            w = n_i / total_n
            denom = max(tau_i, 1e-12)
            for j in range(len(agg_u)):
                agg_u[j] = agg_u[j] + w * ((self._current_weights[j] - w_i[j]) / denom)

        new_weights = [self._current_weights[j] - eta * agg_u[j] for j in range(len(agg_u))]
        self._current_weights = new_weights
        aggregated_parameters = _ndarrays_to_params(new_weights)

        for client_proxy, fit_res, n_i, tau_i, _ in client_items:
            m = fit_res.metrics or {}
